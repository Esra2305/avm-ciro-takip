...
# --- 1. GÜVENLİ BULUT VERİTABANI BAĞLANTISI (PostgreSQL) ---
def vt_baglan():
    # Bağlantı bilgilerini Streamlit Secrets üzerinden güvenli bir şekilde çeker
...
# --- 3. SİSTEMİ BULUTTA BAŞLAT VE TABLOLARI OLUŞTUR ---
def veritabani_hazirla():
    conn = vt_baglan()
    with conn:
        with conn.cursor() as imlec:
            ...
            VALUES ('super_sifre', %s) 
            ON CONFLICT (anahtar) DO NOTHING""", (varsayilan_super_hash,))

            # Çoklu AVM Takip Tablosu (Multi-Tenancy Temeli)
            imlec.execute("""
            ...
            if imlec.fetchone()[0] == 0:
                ...
                ("Merkez Pro AVM", varsayilan_bitis, "DENEME SÜRESİ", admin_sifre_hash))
                yeni_avm_id = imlec.fetchone()[0]
                ...
                for am_id, m_adi, m_kat, m_sifre in varsayilan_magazalar:
                    imlec.execute("INSERT INTO magazalar (avm_id, adi, kat, sifre) VALUES (%s, %s, %s, %s)", (am_id, m_adi, m_kat, m_sifre))
    conn.close()

veritabani_hazirla()

# --- 4. OTURUM HAFIZASI KONTROLLERİ ---
                ...
                if st.button("Mağaza Paneline Giriş Yap"):
                    ...
                    if sifre_hashle(magaza_sifre) == dogru_magaza_hash:
                        st.session_state.update({
                            "giris_yapildi": True, "kullanici_turu": "magaza",
                            "aktif_avm_id": m_avm_sozluk[g_secilen_avm], "aktif_avm_adi": g_secilen_avm,
                            "aktif_magaza_id": magaza_sozluk[secilen_magaza], "aktif_magaza_adi": secilen_magaza
                        })
                        st.rerun()
                    else:
...
# ==============================================================================
    ...
    with col_baslik:
        st.title("🏢 AVM Ciro Yönetim ve Analiz Portalı (SaaS PRO)")
    with col_cikis:
    ...
    if st.session_state["kullanici_turu"] == "super":
        ...
        with st.expander("🔑 Süper Admin Şifremi Değiştir"):
            ...
            if st.button("Süper Şifreyi Güncelle"):
                if yeni_super_sifre.strip() != "":
                    ...
                    with conn:
                        with conn.cursor() as imlec:
                            imlec.execute("UPDATE sistem_ayarlari SET deger = %s WHERE anahtar = 'super_sifre'", (sifre_hashle(yeni_super_sifre),))
                    conn.close()
        ...
        conn = vt_baglan()
        df_avmler = pd.read_sql_query("SELECT id as [AVM ID], avm_adi as [AVM Adı], lisans_bitis as [Lisans Bitiş], odeme_durumu as [Ödeme Durumu] FROM avm_listesi", conn)
        conn.close()
    ...
    # --------------------------------------------------------------------------
        ...
        if secenek == "📊 Kendi Raporlarım & Grafikler":
            ...
            with conn.cursor() as imlec:
                tum_magazalar = imlec.execute("SELECT id, adi, kat FROM magazalar WHERE avm_id = %s", (a_id,)).fetchall() if conn.cursor().execute("SELECT 1") else imlec.fetchall()
                imlec.execute("SELECT magaza_id FROM gunluk_cirolar WHERE avm_id = %s AND tarih = %s", (a_id, tarih_bugun))
                bugun_girenler = [s[0] for s in imlec.fetchall()]
            conn.close()
            ...
            with conn.cursor() as imlec:
                imlec.execute("""
                    ...
                    FROM gunluk_cirolar c 
                    JOIN magazalar m ON c.magaza_id = m.id 
                    WHERE c.avm_id = %s ORDER BY c.id DESC""", (a_id,))
            ...
            if st.button("Yönetici Şifremi Güncelle"):
                if yeni_admin_sifre.strip() != "":
                    ...
                    with conn:
                        with conn.cursor() as imlec:
                            imlec.execute("UPDATE avm_listesi SET yonetici_sifre = %s WHERE id = %s", (sifre_hashle(yeni_admin_sifre), a_id))
                    conn.close()
    ...
    # --------------------------------------------------------------------------
    # ROLE 3: MAĞAZA KULLANICISI PANELİ (Tamamen avm_id ve magaza_id İzoleli)
    # --------------------------------------------------------------------------
        ...
        with sekme_giris:
            ...
            with st.expander("🔑 Giriş Şifremi Değiştir"):
                ...
                if st.button("Şifremi Güncelle"):
                    if y_sifre.strip() != "":
                        ...
                        with conn:
                            with conn.cursor() as imlec:
                                imlec.execute("UPDATE magazalar SET sifre = %s WHERE id = %s", (sifre_hashle(y_sifre), m_id))
                        conn.close()
            ...
            conn = vt_baglan()
            with conn.cursor() as imlec:
                imlec.execute("SELECT COUNT(*) FROM gunluk_cirolar WHERE magaza_id = %s AND tarih = %s", (m_id, tarih_bugun))
            ...
            if giris_var_mi > 0:
                ...
                if st.button("Ciro Verisini Buluta Kilitle"):
                        ...
                        with conn:
                            with conn.cursor() as imlec:
                                imlec.execute("""
                                    ...
                                    VALUES (%s, %s, %s, %s, %s, %s)""", 
                                    (a_id, m_id, tarih_bugun, kdv_dahil, kdv_haric, psycopg2.Binary(foto_byte)))
                        conn.close()
                        st.success("✓ Verileriniz ve Z-Raporu kanıtı bulut veritabanına güvenle işlendi!")
                        st.rerun()
                    else:
        ...
        with sekme_rapor:
            ...
            df_magaza_ozel = pd.read_sql_query("""
                ...
                FROM gunluk_cirolar 
                WHERE magaza_id = %s ORDER BY id DESC""", conn, params=(m_id,))
            conn.close()
